...
export default function FamilySetupScreen() {
  const { createKid, signOut, user } = useAuth()
  ...
  const handleConfirm = async () => {
    ...
    setError('')
    try {
      await createKid(name.trim(), selected)
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-violet-600 via-purple-500 to-indigo-700 flex flex-col items-center justify-center px-4 py-8">
      <div className="text-center mb-8">
        ...
        <p className="text-violet-200 mt-1">Set up your first kid profile</p>
        <p className="text-violet-300/60 text-xs mt-1">{user?.email}</p>
      </div>

      {step === 'name' ? (
        <form onSubmit={handleNameSubmit} className="w-full max-w-xs">
          <div className="bg-white rounded-3xl p-6 shadow-2xl">
            <h2 className="text-xl font-black text-gray-800 mb-1">Kid's name?</h2>
            <p className="text-sm text-gray-400 mb-4">You can add more kids later.</p>
            <input
              autoFocus
              type="text"
              value={name}
        ...
        <div className="w-full max-w-sm">
          ...
          {error && <p className="text-red-300 text-sm font-bold text-center mt-4">{error}</p>}
          ...
          >
            {loading ? 'Setting up...' : 'Start Adventure! 🚀'}
          </button>
        </div>
      )}

      <button onClick={signOut} className="mt-8 flex items-center gap-1 text-white/40 text-xs font-semibold">
